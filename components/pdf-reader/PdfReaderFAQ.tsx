...
// exclusive-open behavior exactly, same approach as components/sheets/SheetsFAQ.tsx.
export function PdfReaderFAQ() {
    ...
    <section style={{ background: '#F7F7F7', paddingTop: '92px', paddingBottom: '92px' }}>
      <PdfReaderRevealSection className="max-w-7xl mx-auto px-6 lg:px-10">
        <div style={{ maxWidth: '820px', margin: '0 auto' }}>
          <div data-pdf-reveal="" style={{ textAlign: 'center', marginBottom: '44px' }}>
            <span style={{ fontSize: '11.5px', fontWeight: '600', letterSpacing: '.14em', color: '#7C3AED', textTransform: 'uppercase' }}>Questions</span>
            <h2 className="pdf-reader-faq-heading" style={{ margin: '12px 0 0', color: '#1B1730' }}>PDF Reader, answered.</h2>
          </div>
          <div data-pdf-reveal="" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {FAQS.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="pdf-reader-faq-question" style={{ color: '#1B1730' }}>{item.q}</span>
                    <span
                      style={{
                        flexShrink: 0,
                        width: '28px',
                  ...
                  {open && (
                    <div className="pdf-reader-faq-answer" style={{ padding: '0 24px 22px', color: '#5B5670', maxWidth: '660px' }}>{item.a}</div>
                  )}
                </div>
              );
            })}
