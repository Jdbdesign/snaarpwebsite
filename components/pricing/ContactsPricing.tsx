...
const PLANS: Plan[] = [
  {
    ...
    slug: 'starter',
    monthlyPrice: 2,
    storageLabel: '500 contacts',
    segment: 'Individuals & freelancers',
    popular: false,
    addonsIcons: [
    ...
    features: ['500 contacts', 'Contact notes & tags', 'Import/export CSV', 'Mail integration', 'Basic search & filter'],
  },
  {
    name: 'Professional',
    slug: 'professional',
    monthlyPrice: 7,
    storageLabel: '10,000 contacts',
    segment: 'Small teams & agencies',
    popular: true,
    addonsIcons: [
      { src: '/assets/icons/search.jpg', name: 'Contacts' },
      { src: '/assets/icons/envelope.jpg', name: 'Mail' },
    ...
    features: ['10,000 contacts', 'Contact notes & tags', 'Import/export CSV', 'Mail integration', 'Advanced search & filter', 'Custom fields', 'Shared contacts', 'Activity timeline', 'Duplicate detection'],
  },
  {
    name: 'Business',
    slug: 'business',
    monthlyPrice: 15,
    storageLabel: 'Unlimited contacts',
    segment: 'Growing organisations',
    popular: false,
    addonsIcons: [
      { src: '/assets/icons/search.jpg', name: 'Contacts' },
      { src: '/assets/icons/envelope.jpg', name: 'Mail' },
    ...
    features: ['Unlimited contacts', 'Contact notes & tags', 'Import/export CSV', 'Mail integration', 'Advanced search & filter', 'Custom fields', 'Shared contacts', 'Activity timeline', 'Duplicate detection', 'API access', 'Audit log'],
  },
];
...
const COMPARE: CompareCategory[] = [
  {
    name: 'Core',
    rows: [
      { label: 'Contact limit', cells: [text('500'), text('10,000'), text('Unlimited')] },
      { label: 'Notes & tags', cells: [check, check, check] },
      { label: 'Import/export CSV', cells: [check, check, check] },
      { label: 'Custom fields', cells: [cross, check, check] },
    ],
  },
  {
    name: 'Organisation',
    rows: [
      { label: 'Basic search & filter', cells: [check, check, check] },
      { label: 'Advanced filters', cells: [cross, check, check] },
      { label: 'Duplicate detection', cells: [cross, check, check] },
      { label: 'Shared contacts', cells: [cross, check, check] },
      { label: 'Activity timeline', cells: [cross, check, check] },
    ],
  },
  {
    name: 'Integrations',
    rows: [
      { label: 'Mail integration', cells: [check, check, check] },
      { label: 'Calendar sync', cells: [check, check, check] },
  ...
  return (
    <div className={`pricing-card${plan.popular ? ' pricing-card-popular' : ''}`}>
      ...
      <ul className="pricing-card-features">
        {plan.features.map((f) => (
          <li key={f}><Check size={16} aria-hidden="true" className="pricing-card-check" /><span>{f}</span></li>
        ))}
      </ul>
...
export function ContactsPricing({ billingCycle = 'monthly' }: { billingCycle?: 'monthly' | 'yearly' }) {
          ...
          <table className="compare-table">
            <thead>
              <tr>
                ...
                {PLANS.map((p) => (
                  <th scope="col" key={p.slug} className={p.popular ? 'is-popular' : undefined}>
                    {p.popular && <span className="compare-popular-badge">Popular</span>}
                    <span className="compare-plan-name">{p.name}</span>
                    <span className="compare-plan-price"><Price amount={p.monthlyPrice} /><span className="compare-plan-price-suffix">/mo</span></span>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {COMPARE.map((cat) => (
                <Fragment key={cat.name}>
                  <tr className="compare-category-row"><th scope="colgroup" colSpan={PLANS.length + 1}>{cat.name}</th></tr>
                  {cat.rows.map((row) => (
                    <tr key={row.label}>
                      <th scope="row" className="compare-col-feature">{row.label}</th>
                      {row.cells.map((cell, i) => (
                        <td key={PLANS[i].slug} className={PLANS[i].popular ? 'is-popular' : undefined}>
