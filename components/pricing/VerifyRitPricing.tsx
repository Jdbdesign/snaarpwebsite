...
type Plan = {
  name: string; slug: string; monthlyPrice: number;
  storageLabel: string; segment: string; popular: boolean;
...
const PLANS: Plan[] = [
  {
    ...
    slug: 'starter',
    monthlyPrice: 4,
    storageLabel: '500 verifications/mo',
    segment: 'Small lists & freelancers',
    popular: false,
    addonsIcons: [
    ...
    features: ['500 verifications per month', 'Single email check', 'Bulk CSV upload', 'Syntax & domain validation', 'Disposable email detection'],
  },
  {
    name: 'Professional',
    slug: 'professional',
    monthlyPrice: 15,
    storageLabel: '5,000 verifications/mo',
    segment: 'Growing teams & marketers',
    popular: true,
    addonsIcons: [
      { src: '/assets/icons/apps-sendrit.jpg', name: 'Sendrit' },
      { src: '/assets/icons/envelope.jpg', name: 'Mail' },
    ...
    features: ['5,000 verifications per month', 'Single & bulk check', 'CSV upload & export', 'Full validation suite', 'Disposable email detection', 'Catch-all detection', 'Risk scoring', 'API access'],
  },
  {
    name: 'Enterprise',
    slug: 'enterprise',
    monthlyPrice: 39,
    storageLabel: '50,000 verifications/mo',
    segment: 'High-volume senders',
    popular: false,
    addonsIcons: [
      { src: '/assets/icons/apps-sendrit.jpg', name: 'Sendrit' },
      { src: '/assets/icons/envelope.jpg', name: 'Mail' },
    ...
    features: ['50,000 verifications per month', 'Single & bulk check', 'CSV upload & export', 'Full validation suite', 'Disposable email detection', 'Catch-all detection', 'Risk scoring', 'API access', 'Webhook integrations', 'Dedicated support'],
  },
];
...
const COMPARE: CompareCategory[] = [
  { name: 'Core', rows: [
    { label: 'Verifications per month', cells: [text('500'), text('5,000'), text('50,000')] },
    { label: 'Single email check', cells: [check, check, check] },
    { label: 'Bulk CSV upload', cells: [check, check, check] },
    { label: 'CSV export', cells: [cross, check, check] },
  ]},
  { name: 'Validation', rows: [
    { label: 'Syntax & domain check', cells: [check, check, check] },
    { label: 'MX record validation', cells: [check, check, check] },
    { label: 'Disposable email detection', cells: [check, check, check] },
    { label: 'Catch-all detection', cells: [cross, check, check] },
    { label: 'Risk scoring', cells: [cross, check, check] },
    { label: 'Mailbox verification', cells: [cross, check, check] },
  ]},
  { name: 'Integrations', rows: [
    { label: 'Sendrit integration', cells: [check, check, check] },
    { label: 'API access', cells: [cross, check, check] },
    { label: 'Webhook integrations', cells: [cross, cross, check] },
    { label: 'CRM sync', cells: [cross, cross, check] },
  ]},
  { name: 'Support', rows: [
    { label: 'Email support', cells: [check, check, check] },
    { label: 'Priority support', cells: [cross, check, check] },
    { label: 'Dedicated support', cells: [cross, cross, check] },
  ]},
];
...
export function VerifyRitPricing({ billingCycle = 'monthly' }: { billingCycle?: 'monthly' | 'yearly' }) {
      ...
      <section className="compare-section max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-24" ref={compareRef}>
        ...
        <div className="compare-scroll" data-reveal>
          <table className="compare-table">
            <thead><tr><th scope="col" className="compare-col-feature">Features</th>
              {PLANS.map((p) => (<th scope="col" key={p.slug} className={p.popular ? 'is-popular' : undefined}>{p.popular && <span className="compare-popular-badge">Popular</span>}<span className="compare-plan-name">{p.name}</span><span className="compare-plan-price"><Price amount={p.monthlyPrice} /><span className="compare-plan-price-suffix">/mo</span></span></th>))}
            </tr></thead>
            <tbody>{COMPARE.map((cat) => (<Fragment key={cat.name}><tr className="compare-category-row"><th scope="colgroup" colSpan={PLANS.length + 1}>{cat.name}</th></tr>
              {cat.rows.map((row) => (<tr key={row.label}><th scope="row" className="compare-col-feature">{row.label}</th>
                {row.cells.map((cell, i) => (<td key={PLANS[i].slug} className={PLANS[i].popular ? 'is-popular' : undefined}><div className="compare-cell">{cell.kind === 'check' && <Check size={18} aria-hidden="true" className="compare-check" />}{cell.kind === 'cross' && <X size={16} aria-hidden="true" className="compare-cross" />}{cell.kind === 'text' && <span className="compare-value">{cell.value}</span>}</div></td>))}
              </tr>))}</Fragment>))}</tbody>
          </table>
        </div>
      </section>
