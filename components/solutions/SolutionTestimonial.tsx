...
export interface SolutionTestimonialItem {
  quote: string;
  initials: string;
  avatarBg: string;
  avatarColor: string;
...
interface SolutionTestimonialProps {
  eyebrow: string;
  heading: string;
  testimonials: SolutionTestimonialItem[];
  /** Shown on each card under fictional placeholder testimonials so they
   * don't ship unnoticed as if they were real customer quotes. */
  todoNote?: string;
}

// Three-card grid, ported from the CRM product page's testimonial layout
// (components/crm/CrmTestimonials.tsx) so solution pages match it exactly —
// only the copy changes per solution.
export function SolutionTestimonial({ eyebrow, heading, testimonials, todoNote }: SolutionTestimonialProps) {
  return (
    <section style={{ background: '#fff', paddingTop: '92px', paddingBottom: '92px' }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <SolutionRevealSection reveal style={{ textAlign: 'center', maxWidth: '600px', margin: '0 auto 48px' }}>
          <span style={{ fontSize: '11.5px', fontWeight: 600, letterSpacing: '.14em', color: '#7C3AED', textTransform: 'uppercase' }}>{eyebrow}</span>
          <h2 className="solution-section-heading" style={{ margin: '12px 0 0', color: '#1B1730' }}>{heading}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '22px' }}>
          {testimonials.map((t, i) => (
            ...
            >
              ...
              <p style={{ fontSize: '16px', lineHeight: 1.55, fontWeight: 500, color: '#211C36', margin: '16px 0 0' }}>&ldquo;{t.quote}&rdquo;</p>
              ...
              {todoNote && (
                <span style={{ position: 'absolute', bottom: '12px', right: '14px', fontSize: '9px', fontWeight: 600, letterSpacing: '.06em', color: '#C4BDD6', textTransform: 'uppercase' }}>{todoNote}</span>
              )}
            </SolutionRevealSection>
          ))}
        </div>
