...
/* Shared screen-preview chrome — the same "traffic-light dots + app pill"
   window frame used by AI Compose's ComposeMockup, so every product page's
   screen preview reads as one family. */
export function ScreenChrome({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="relative w-full rounded-3xl border bg-card p-5 shadow-2xl md:p-7">
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-2"><span className="size-2.5 rounded-full bg-primary" /><span className="size-2.5 rounded-full bg-border" /><span className="size-2.5 rounded-full bg-border" /></div>
        <span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-primary">{label}</span>
      </div>
      {children}
    </div>
  );
  ...
  return (
    <ScreenChrome label="Work Drive">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border bg-background p-4"><FileText className="size-7 text-primary" /><p className="mt-5 text-sm font-semibold">Project brief.pdf</p><p className="text-xs text-muted-foreground">2.4 MB</p></div>
        <div className="rounded-2xl border bg-background p-4"><FileImage className="size-7 text-primary" /><p className="mt-5 text-sm font-semibold">Homepage mockup.png</p><p className="text-xs text-muted-foreground">8.1 MB</p></div>
        <div className="rounded-2xl bg-foreground p-4 text-background"><p className="text-xs text-background/60">Storage used</p><p className="mt-2 text-2xl font-bold">12.8 GB</p><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-background/20"><div className="h-full w-3/5 rounded-full bg-primary" /></div><p className="mt-2 text-[11px] text-background/60">of 20 GB shared</p></div>
      </div>
    </ScreenChrome>
  );
}
...
export function SearchVisual() {
  return (
    <ScreenChrome label="Search">
      <div className="flex items-center gap-3 rounded-2xl border bg-background p-4"><Search className="size-5 text-primary" /><span className="text-sm">project brief</span><kbd className="ml-auto rounded-md bg-muted px-2 py-1 text-xs text-muted-foreground">⌘ K</kbd></div>
      <div className="mt-4 flex flex-col gap-2">{['Project brief.pdf', 'Project brief notes.doc', 'Archived brief v1.pdf'].map((file, i) => <div key={file} className={`flex items-center gap-3 rounded-xl p-3 ${i === 0 ? 'bg-primary/8' : 'bg-muted/50'}`}><FileText className="size-5 text-primary" /><span className="text-sm font-medium">{file}</span><span className="ml-auto text-xs text-muted-foreground">{i + 1}d ago</span></div>)}</div>
    </ScreenChrome>
  );
}
...
export function StorageVisual() {
  return (
    <ScreenChrome label="Storage">
      <div className="flex items-center justify-between"><div><p className="text-xs text-muted-foreground">Shared storage</p><p className="text-3xl font-bold">12.8 GB</p></div><span className="rounded-full bg-accent-mint px-3 py-1.5 text-xs font-semibold">Healthy</span></div>
      <div className="mt-7 flex h-32 items-end gap-3">{[38, 62, 48, 84, 66, 92, 76].map((height, i) => <div key={i} className="flex-1 rounded-t-xl bg-primary/15" style={{ height: `${height}%` }}><div className="h-full rounded-t-xl bg-primary" style={{ opacity: .35 + i * .07 }} /></div>)}</div>
      <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground"><History className="size-4" /> Files stay put when your plan changes</div>
    </ScreenChrome>
  );
}
