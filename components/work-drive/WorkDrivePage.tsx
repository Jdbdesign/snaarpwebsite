...
import { ArrowRight, Check, ChevronDown, CloudUpload, Files, FolderSearch, History, MessageSquare, RefreshCw, Search, ShieldCheck, Tags, Users } from 'lucide-react';
import { buttonVariants } from '@/components/work-drive/Button';
...
const faqs: { q: ReactNode; a: ReactNode }[] = [
  { q: 'How much storage do I get?', a: 'Every plan includes a shared storage pool for your whole team. See the pricing page for exact limits per plan.' },
...
function Kicker({ children, className = '' }: { children: React.ReactNode; className?: string }) { return <span className={`text-xs font-bold uppercase tracking-[.18em] text-primary ${className}`}>{children}</span>; }
...
export function WorkDrivePage() {
  return <div className="work-drive min-h-screen overflow-x-clip bg-background text-foreground"><main>
    ...
    <section id="features" className="shared-work-section section-pad overflow-hidden"><div className="mx-auto max-w-7xl px-6 lg:px-10">
      <div className="reveal mx-auto flex max-w-2xl flex-col items-center gap-5 text-center"><Kicker>Built for shared work</Kicker><h2 className="text-balance text-4xl font-bold tracking-tight md:text-6xl">One place. Every file. <span className="text-primary">Everyone in sync.</span></h2><p className="text-pretty text-base leading-relaxed text-muted-foreground md:text-lg">Work Drive keeps storage, sharing, history, and search connected, so your team can move together without losing context.</p></div>
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        <article className="bento-card reveal flex flex-col overflow-hidden p-6 md:p-8 lg:col-span-3" aria-labelledby="shared-storage-title">
          ...
          <h3 id="shared-storage-title" className="mt-5 text-2xl font-bold tracking-tight">One shared storage pool</h3>
        ...
        <article className="bento-card bento-card-accent reveal flex flex-col overflow-hidden p-6 md:p-8 lg:col-span-3" style={{ animationDelay: '80ms' }} aria-labelledby="attach-title">
          ...
          <h3 id="attach-title" className="mt-5 text-2xl font-bold tracking-tight">Attach from anywhere</h3>
        ...
        <article className="bento-card reveal group flex flex-col overflow-hidden p-6 md:p-8 lg:col-span-2" style={{ animationDelay: '140ms' }} aria-labelledby="history-title">
          ...
          <h3 id="history-title" className="mt-5 text-xl font-bold tracking-tight">Every version, remembered</h3>
        ...
        <article className="bento-card reveal flex flex-col overflow-hidden p-6 md:p-8 lg:col-span-2" style={{ animationDelay: '200ms' }} aria-labelledby="search-title">
          ...
          <h3 id="search-title" className="mt-5 text-xl font-bold tracking-tight">Search beyond file names</h3>
        ...
        <article className="bento-card reveal flex flex-col overflow-hidden p-6 md:p-8 md:col-span-2 lg:col-span-2" style={{ animationDelay: '260ms' }} aria-labelledby="live-title">
          ...
          <h3 id="live-title" className="mt-5 text-xl font-bold tracking-tight">In sync, in real time</h3>
    ...
    <section id="feature-details" className="section-pad bg-muted/55"><div className="mx-auto flex max-w-7xl flex-col gap-24 px-6 lg:px-10">
      <div className="grid items-center gap-12 lg:grid-cols-2"><div className="reveal flex flex-col gap-6"><Kicker>Everything, findable</Kicker><h2 className="text-balance text-3xl font-bold tracking-tight md:text-5xl">Stop hunting for &apos;that file someone sent.&apos;</h2><p className="text-base leading-relaxed text-muted-foreground">Files end up scattered across email attachments, chat threads, and random folders. Work Drive keeps everything in one searchable place, connected to the rest of the Stack.</p><NumberList items={[['One place, not five.', 'Stop digging through old emails and chat threads to find a file.'], ['Attach without downloading first.', 'Pull a file into an email or a Teams message directly from Work Drive.'], ['Never lose the old version.', 'Version history means you can always go back.']]} /></div><div className="reveal reveal-delay"><SearchVisual /></div></div>
      <div className="grid items-center gap-12 lg:grid-cols-2"><div className="reveal order-2 lg:order-1"><StorageVisual /></div><div className="reveal order-1 flex flex-col gap-6 lg:order-2"><Kicker>Room to grow</Kicker><h2 className="text-balance text-3xl font-bold tracking-tight md:text-5xl">Storage that scales with your team</h2><NumberList items={[['One shared pool.', 'Storage is pooled across your whole team, not capped per person.'], ["See what's using space.", "A clear storage breakdown shows what's taking up room, so nothing surprises you."], ['Upgrade without disruption.', 'Move to a bigger plan without touching a single file or folder.']]} /></div></div>
    </div></section>

    <section id="faq" className="section-pad bg-muted/55"><div className="mx-auto max-w-7xl px-6 lg:px-10">
      <div className="mx-auto flex max-w-3xl flex-col items-center gap-4 text-center"><Eyebrow>Questions</Eyebrow><h2 className="text-balance text-3xl font-bold tracking-tight md:text-5xl">Work Drive, answered.</h2></div>
...
function NumberList({ items }: { items: string[][] }) { return <ol className="flex flex-col gap-5">{items.map(([title, copy], i) => <li key={title} className="flex gap-4"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-primary text-xs font-bold text-primary-foreground">{i + 1}</span><p className="text-sm leading-relaxed text-muted-foreground"><strong className="block text-base text-foreground">{title}</strong>{copy}</p></li>)}</ol>; }
